...
if [ ! -f /var/lib/samba/registry.tdb ]; then
  if [ -z "$ADMIN_PASS" ]; then
    echo 'ADMIN_PASS env var not set'
    exit 1
  fi
  ...
  if [ $DOMAIN_ACTION == provision ]; then
    ...
    if [ -n "$DOMAIN_SID" ]; then
      PROVISION_OPTS="$PROVISION_OPTS --domain-sid=$DOMAIN_SID"
    fi
    if [ -n "$MACHINE_PASS" ]; then
      PROVISION_OPTS="$PROVISION_OPTS --machinepass='$MACHINE_PASS'"
    fi
